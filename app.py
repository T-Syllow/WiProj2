...
reduced_transaction_data_data = pd.read_csv(data_folder + "reduced_transaction_data.csv", sep=",",  encoding="utf8")
...
# for column in reduced_transaction_data_data.columns:
#     # prepare a set for all units that are found in this column
#     units = set()
#     # search for units
#     for value in reduced_transaction_data_data[column][1:]:  # skip first row
#         unit = get_unit(value)
#         if unit:
#             units.add(unit)
...
#         reduced_transaction_data_data[column] = reduced_transaction_data_data[column].apply(remove_unit)
#     elif len(units) > 1:
...
# #reduced_transaction_data_data['vitamin_c'] = pd.to_numeric(reduced_transaction_data_data['vitamin_c'], errors='coerce')
# top_10_vitamin_c = reduced_transaction_data_data.nlargest(10, 'vitamin_c')
...
#     ]),
#     dbc.Row([
#         dash_table.DataTable(data=reduced_transaction_data_data.to_dict('records'), page_size=10, style_table={'overflowX': 'auto'}, id='tbl'),
#     ]),
#     dbc.Row([
#         dbc.Tabs([
...
#     return 'Value: ' + str(reduced_transaction_data_data.iloc[active_cell['row'], active_cell['column']]) + ', Name: ' + str(reduced_transaction_data_data.iloc[active_cell['row'], 1]) if active_cell else "Click the table"
...
#     if x_value != y_value:
#         fig=px.scatter(reduced_transaction_data_data, x=x_value, y=y_value, color="niacin", hover_data=['name'], trendline="ols")
#         fig.update_layout()
#         return fig, False, x_value, y_value
#     fig=px.scatter(reduced_transaction_data_data, x='sodium', y='vitamin_c', color="niacin", hover_data=['name'], trendline="ols")
...
#     fig=px.histogram(reduced_transaction_data_data, x=col_chosen,
